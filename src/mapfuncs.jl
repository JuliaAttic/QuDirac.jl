...
Base.filter!(f::Function, opc::DualOpSum) = (filter!((k,v)->f(reverse(k),v'), dict(opc)); return opc)
...
Base.filter(f::Function, opc::DualOpSum) = similar(opc, filter((k,v)->f(reverse(k),v'), dict(opc)))
...
maplabels(f::Union(Function,DataType), opc::DualOpSum) = OpSum(ptype(opc), mapkeys(label->f(reverse(label)), dict(op)))
...
Base.map(f::Union(Function,DataType), opc::DualOpSum) = OpSum(ptype(opc), mapkv(kv->f(reverse(kv[1]), kv[2]'), dict(d)))
