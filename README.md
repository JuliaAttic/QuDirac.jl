...
julia> a = GenericOp(label->(sqrt(label[1]),{label[1]-1}), k) # lower operator on basis of k
...
- `filter`/`filter!` are supported on both the labels and coefficients of an operator/state
