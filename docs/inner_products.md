...
Let's say I know for a fact that my use of `SumInner` will be restricted to cases where the state labels hold `Int64`s. Then, it's easy to see that evaluations using `SumInner` will always produce `Int64`s. I can share this assumption with QuDirac by defining the following:
